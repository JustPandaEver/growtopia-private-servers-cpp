...
void sendTileUpdate(int x, int y, int tile, int causedBy, ENetPeer* peer)
{
	...
	if (tile != 5924 && tile != 2534 && tile != 5822 && tile != 6642 && tile != 5204 && tile == 4174 && tile != 6420 && tile != 5992 && tile != 1884 && tile != 9942 && tile != 6322 && tile != 1152 && tile != 7742 && tile != 2225 && tile != 6636 && tile != 2668 && tile != 5552 && tile != 1153 && tile != 6634 && tile != 5552 && tile != 8532 && tile != 7723 && tile != 5531 && world->name == "KSNGU") fixedreturn(peer);
					...
					if (!((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						if (!((PlayerInfo*)(event.peer->data))->haveGrowId)
						{
							...
							if (logStatus == 1) {
								...
								if (b) {
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This account is banned!"));
									ENetPacket * packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(peer, 0, packet);
