...
int PlayerDB::playerLogin(ENetPeer* peer, string username, string password) {
	...
	if (ifs.is_open()) {
		...
		if (verifyPassword(password, pss)) {
			...
			((PlayerInfo*)(peer->data))->cloth_hair = j["ClothBack"];
			((PlayerInfo*)(peer->data))->cloth_hair = j["ClothHand"];
			((PlayerInfo*)(peer->data))->cloth_hair = j["ClothFace"];
			((PlayerInfo*)(peer->data))->cloth_hair = j["ClothShirt"];
			((PlayerInfo*)(peer->data))->cloth_hair = j["ClothPants"];
			((PlayerInfo*)(peer->data))->cloth_hair = j["ClothNeck"];
			((PlayerInfo*)(peer->data))->cloth_hair = j["ClothHair"];
			((PlayerInfo*)(peer->data))->cloth_hair = j["ClothFeet"];
			((PlayerInfo*)(peer->data))->cloth_hair = j["ClothMask"];
...
void SendPacketRaw(int a1, void *packetData, size_t packetDataSize, void *a4, ENetPeer* peer, int packetFlag)
{
	...
	int getPlyersCount(string name)
	{
		int count = 0;
		ENetPeer * currentPeer;
		for (currentPeer = server->peers;
	...
	void battlebus() {
		...
		while (battlebus) {
			...
			{
				counter++;
				test = std::cos(counter / 1000.0);
				test2 = std::sin(counter / 1000.0);
			}
			timer.stop();
...
// #if DEBUG
				...
				++currentPeer)
			{
				...
				if (((PlayerInfo*)(currentPeer->data))->currentWorld != "EXIT" && ((PlayerInfo*)(currentPeer->data))->isOut == false)
				{
					GamePacket p2 = packetEnd(appendFloat(appendString(createPacket(), "OnSetPos"), ((PlayerInfo*)(currentPeer->data))->x + 50, ((PlayerInfo*)(currentPeer->data))->y));
...
// #endif
	...
	void sendTileUpdate(int x, int y, int tile, int causedBy, ENetPeer* peer)
	{
		...
		if (tile == 196)
		{
			GamePacket p4 = packetEnd(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), ((PlayerInfo*)(peer->data))->netID), "`oYou drunk `eShield `oPotion``!"));
			ENetPacket * packet4 = enet_packet_create(p4.data,
				p4.len,
				ENET_PACKET_FLAG_RELIABLE);
		...
		if (tile == 544 || tile == 510620 || tile == 4520 || tile == 382 || tile == 3116 || tile == 4520 || tile == 1792 || tile == 5666 || tile==2994 || tile==4368) return;
		if (tile == 5708 || tile == 5709 || tile == 5780 || tile == 5781 || tile == 5782 || tile == 5783 || tile == 5784 || tile == 5785 || tile == 5710 || tile == 5711 || tile == 5786 || tile == 5787 || tile == 5788 || tile == 5789 || tile == 5790 || tile == 5791 || tile == 6146 || tile == 6147 || tile == 6148 || tile == 6149 || tile == 6150 || tile == 6151 || tile == 6152 || tile == 6153 || tile == 5670 || tile == 5671 || tile == 5798 || tile == 5799 || tile == 5800 || tile == 5801 || tile == 5802 || tile == 5803 || tile == 5668 || tile == 5669 || tile == 5792 || tile == 5793 || tile == 5794 || tile == 5795 || tile == 5796 || tile == 5797 || tile == 544 || tile == 546000 || tile == 4520 || tile == 382 || tile == 3116 || tile == 1792 || tile == 5666 || tile == 2994 || tile == 4368) return;
	...
	void sendPlayerLeave(ENetPeer* peer, PlayerInfo* player)
	{
		...
		GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`5<`w" + player->displayName + "`` left, `w" + std::to_string(getPlyersCount(player->currentWorld)) + "`` others here>``"));
		for (currentPeer = server->peers;
			currentPeer < &server->peers[server->peerCount];
			++currentPeer)
		{
	...
	{
		((PlayerInfo*)(peer->data))->invincible = false;		
		{
	...
	void sendWorldOffers(ENetPeer* peer)
	{
		...
		for (int i = 0; i < worlds.size(); i++) {
			worldOffers += "add_floater|"+worlds[i].name+"|"+std::to_string(getPlyersCount(worlds[i].name))+"|0.55|3529161471\n";
		}
...
{
	cout << "Growtopia battle royale server (c) PlayIngoHD Gaming" << endl;
	cout << "Server template by (c) Growtopia Noobs." << endl;
	enet_initialize();
	if (atexit(saveAllWorlds)) {
		cout << "Worlds won't be saved for this session..." << endl;
	}

	addAdmin("playingo", "akinatornerd1337!", 999);
	thread t1(battlebus);
	t1.detach();
		...
		case ENET_EVENT_TYPE_RECEIVE:
		{
			...
			case 2:
			{
				...
				if (cch.find("action|buy") == 0)
				{
					...
					GamePacket p3 = packetEnd(appendString(appendString(createPacket(), "OnStorePurchaseResult"), "Bought `6Ancestral of Dimension`w (To level up, buy ancestral again!)"));
				...
				if (cch.find("action|dialog_return") == 0)
				{
					...
					if (isRegisterDialog) {
						...
						{
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Please use normal characters and dont use color codes or symbols in name!``"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
				...
				if (cch.find("text|") != std::string::npos){
					...
					if (str == "/ghost")
					{
						if (isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass))
						{
							...
							((PlayerInfo*)(peer->data))->skinColor = -159;
					...
					else if (str.substr(0, 6) == "/find ")
					{
						...
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Help >> /help /skins /weapons /trophies /stats /radio /gem /unequip"));
						ENetPacket * packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
					...
					if (isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass))
					{
						...
						if (isSuperAdmin(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass))
						{
							...
							if (!pData->isGhost) {
								...
								sendConsoleMsg(peer, "`oold crappy invis mod added");
								...
								sendConsoleMsg(peer, "`oold crappy invis mod removed");
						...
						if (((PlayerInfo*)(peer->data))->radio) {
							p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "RADIO! You won't see broadcasts anymore."));
				...
				if (!((PlayerInfo*)(event.peer->data))->isIn)
				{
					...
					if (!((PlayerInfo*)(event.peer->data))->haveGrowId)
					{
						...
						((PlayerInfo*)(event.peer->data))->displayName = "Royale_ " + PlayerDB::fixColors(((PlayerInfo*)(event.peer->data))->requestedName.substr(0, ((PlayerInfo*)(event.peer->data))->requestedName.length()>15?15:((PlayerInfo*)(event.peer->data))->requestedName.length()));
					}
					else if (((PlayerInfo*)(event.peer->data))->tankIDName.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789") != string::npos) {
						enet_peer_disconnect_later(peer, 0);
						...
						if (logStatus == 1) {
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`r[Growtopia Battle Royale]: Login successful! Joining world to find matchmaking...``"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Invalid / Incorrect username or password.``"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
				...
				if(pStr.substr(0, 17) == "action|enter_game" && !((PlayerInfo*)(event.peer->data))->isIn)
				{
					...
					GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "[Please wait while your match is being found!]"));
					ENetPacket * packet = enet_packet_create(p.data,
						p.len,
						ENET_PACKET_FLAG_RELIABLE);
					enet_peer_send(peer, 0, packet);
				...
				while (std::getline(ss, to, '\n')) {
					...
					if (id == "name" && isJoinReq)
					{
...
#endif
						try {
							WorldInfo info = worldDB.get("BATTLEROYALE");
							sendWorld(peer, &info);
						...
						if (id == "action")
						{
							...
							if (act == "quit_to_exit")
							{
								if (((PlayerInfo*)(peer->data))->killedBy == "")
								{
								...
								{
									((PlayerInfo*)(peer->data))->displayName == "`e(Spectator)``";
									sendPlayerToWorld(peer, (PlayerInfo*)(peer->data), "BATTLEROYALE");
			...
			{
				{
					...
					if (tankUpdatePacket)
					{
						if (((PlayerInfo*)(peer->data))->isDead)
						{
							GamePacket p2f = packetEnd(appendIntx(appendString(createPacket(), "OnSetFreezeState"), 1));
							memcpy(p2f.data + 8, &(((PlayerInfo*)(peer->data))->netID), 4);
							ENetPacket * packet2f = enet_packet_create(p2f.data,
								p2f.len,
								ENET_PACKET_FLAG_RELIABLE);

							enet_peer_send(peer, 0, packet2f);

							((PlayerInfo*)(peer->data))->noEyes = true;
							((PlayerInfo*)(peer->data))->skinColor = 0;
							sendState(peer);
							sendClothes(peer);
						...
						int   cast_x = static_cast<int>(pMov->x);
						int   cast_y = static_cast<int>(pMov->y);

						//int mov_xy = (cast_x + cast_y) / 32;
						int pun_xy = pMov->punchX + pMov->punchY;
						((PlayerInfo*)(peer->data))->movXY = (cast_x + cast_y) / 32;
						((PlayerInfo*)(peer->data))->punchXY = pMov->punchX + pMov->punchY;
							...
							++currentPeer)
						{
							if (currentPeer->state != ENET_PEER_STATE_CONNECTED)
								...
								if (((PlayerInfo*)(currentPeer->data))->killedBy == ((PlayerInfo*)(peer->data))->rawName)
								{
									if (((PlayerInfo*)(currentPeer->data))->haveGrowId && ((PlayerInfo*)(peer->data))->haveGrowId)
									{
										if (pMov->punchX != 0 && pMov->punchX != 1 && pMov->punchX != 2 && pMov->punchY != 0 && pMov->punchY != 1 && pMov->punchY != 2) {
											// is spectating
											GamePacket p2 = packetEnd(appendFloat(appendString(createPacket(), "OnSetPos"), pMov->x, pMov->y));
											memcpy(p2.data + 8, &(((PlayerInfo*)(currentPeer->data))->netID), 4);
											ENetPacket * packet2 = enet_packet_create(p2.data,
												p2.len,
												ENET_PACKET_FLAG_RELIABLE);
											...
											GamePacket p7 = packetEnd(appendString(appendString(createPacket(), "OnNameChanged"), ""));
											memcpy(p7.data + 8, &(((PlayerInfo*)(currentPeer->data))->netID), 4); // ffloor

											//((PlayerInfo*)(currentPeer->data))->displayName = ((PlayerInfo*)(currentPeer->data))->displayName + "(Health: " + std::to_string(chealth) + ")";
											...
											GamePacket pto = packetEnd(appendString(appendString(createPacket(), "OnTextOverlay"), "`wYou are spectating.``"));
											//memcpy(p2.data + 8, &(((PlayerInfo*)(currentPeer->data))->netID), 4);
							...
							//cout << "player pos xy (conv): " << mov_xy << endl;
							if (pMov->punchX != -1 && pMov->punchY != -1 && pMov->punchX != 0 && pMov->punchX != 1 && pMov->punchX != 2) {

								
									cout << "player punch xy (conv): " << pun_xy << endl;
									...
									if (((PlayerInfo*)(peer->data))->punchXY == ((PlayerInfo*)(currentPeer->data))->movXY)
									{
										if (((PlayerInfo*)(peer->data))->rawName != ((PlayerInfo*)(currentPeer->data))->rawName)
										{
											cout << "punched player!" << endl;
											

											if (((PlayerInfo*)(currentPeer->data))->isDead == false)
											{
												int chealth = ((PlayerInfo*)(currentPeer->data))->health = ((PlayerInfo*)(currentPeer->data))->health - 5;
												GamePacket p7 = packetEnd(appendString(appendString(createPacket(), "OnNameChanged"), ((PlayerInfo*)(currentPeer->data))->displayName + " (Health: " + std::to_string(chealth) + ")"));
												memcpy(p7.data + 8, &(((PlayerInfo*)(currentPeer->data))->netID), 4); // ffloor
											...
											if (((PlayerInfo*)(currentPeer->data))->health == 0 || ((PlayerInfo*)(currentPeer->data))->health < 0)
											{			
												if (((PlayerInfo*)(currentPeer->data))->isDead == false && ((PlayerInfo*)(currentPeer->data))->invincible == false)
												{
													((PlayerInfo*)(currentPeer->data))->isDead = true;
													((PlayerInfo*)(peer->data))->totalKills = ((PlayerInfo*)(peer->data))->totalKills + 1;

																										
													GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`wKilled +1 player! Total Kills: " + std::to_string(((PlayerInfo*)(peer->data))->totalKills)));
													ENetPacket * packet = enet_packet_create(p.data,
														p.len,
														ENET_PACKET_FLAG_RELIABLE);
													enet_peer_send(peer, 0, packet);

													world->deadppl = world->deadppl + 1;

													GamePacket pp = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/atomic_button.rttex"), "`oYou've been `4killed!``"), "audio/male_scream.wav"), 0));
													ENetPacket * packetp = enet_packet_create(pp.data,
														pp.len,
														ENET_PACKET_FLAG_RELIABLE);

													enet_peer_send(currentPeer, 0, packetp);

													((PlayerInfo*)(currentPeer->data))->killedBy = ((PlayerInfo*)(peer->data))->rawName;
													((PlayerInfo*)(currentPeer->data))->skinColor = 0;
													((PlayerInfo*)(currentPeer->data))->noEyes = true;
							...
							if (((PlayerInfo*)(peer->data))->rawName != ((PlayerInfo*)(currentPeer->data))->rawName)
							{
								if (((PlayerInfo*)(currentPeer->data))->isDead == true)
								{
									

									if (getPlyersCount(((PlayerInfo*)(currentPeer->data))->currentWorld) == world->deadppl - 1)
									{
										GamePacket pi = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/cash_icon_overlay.rttex"), "`2Victory `oRoyale!``"), "audio/mp3/death_metal_radio.mp3"), 0));
										ENetPacket * packeti = enet_packet_create(pi.data,
											pi.len,
											ENET_PACKET_FLAG_RELIABLE);

										enet_peer_send(peer, 0, packeti);

										GamePacket px = packetEnd(appendInt(appendString(appendString(appendString(appendString(createPacket(), "OnAddNotification"), "interface/cash_icon_overlay.rttex"), "`w" + ((PlayerInfo*)(peer->data))->displayName + " `2won`5 the game! `4Resetting game..."), "audio/mp3/death_metal_radio.mp3"), 0));
										ENetPacket * packetx = enet_packet_create(px.data,
											px.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(currentPeer, 0, packetx);

										((PlayerInfo*)(currentPeer->data))->invincible = true;
										((PlayerInfo*)(peer->data))->invincible = true;
										((PlayerInfo*)(currentPeer->data))->isDead = false;
										((PlayerInfo*)(peer->data))->isDead = false;
										((PlayerInfo*)(peer->data))->totalKills = 0;
										((PlayerInfo*)(currentPeer->data))->totalKills = 0;
										((PlayerInfo*)(currentPeer->data))->noEyes = false;
										...
										((PlayerInfo*)(currentPeer->data))->health = 100;
										((PlayerInfo*)(currentPeer->data))->killedBy = "";
										((PlayerInfo*)(peer->data))->killedBy = "";
										sendState(peer);
										sendState(currentPeer);
										sendClothes(peer);
										sendClothes(currentPeer);
										...
										sendPlayerToWorld(peer, (PlayerInfo*)(peer->data), "BATTLEROYALE");
										sendPlayerToWorld(currentPeer, (PlayerInfo*)(currentPeer->data), "BATTLEROYALE");
									}
								}
							}
							}
						...
						if (data2->packetType == 7)
						{
							...
							((PlayerInfo*)(event.peer->data))->isOut = true;
							GamePacket p2 = packetEnd(appendFloat(appendString(createPacket(), "OnSetPos"), ((PlayerInfo*)(event.peer->data))->x, ((PlayerInfo*)(event.peer->data))->y + 50));
							memcpy(p2.data + 8, &(((PlayerInfo*)(event.peer->data))->netID), 4);
							ENetPacket * packet2 = enet_packet_create(p2.data,
								p2.len,
								ENET_PACKET_FLAG_RELIABLE);
							...
							GamePacket p2x = packetEnd(appendIntx(appendString(createPacket(), "OnSetFreezeState"), 0));
							memcpy(p2.data + 8, &(((PlayerInfo*)(peer->data))->netID), 4);
