...
struct PlayerInfo {
	...
	bool fullInvisible = false; //8686986
...
AWorld WorldDB::get2(string name) {
	...
	if (ifs.is_open()) {
		...
		cout << "GAY CREATED  WORLD CALLED :" << info.name << "." << endl;
...
void addVip(string username, string password, int level)
{
	Vip vip;
...
int getAdminVip(string username, string password) {
	for (int i = 0; i < vips.size(); i++) {
		Vip vip = vips[i];
		if (vip.username == username && vip.password == password) {
			return vip.level;
		}
	}
	return 0;
...
bool isHamumu(string username, string password) {
	for (int i = 0; i < admins.size(); i++) {
		Admin admin = admins[i];
...
bool isModeratorFlag(string username, string password) {
	for (int i = 0; i < admins.size(); i++) {
		Admin admin = admins[i];
...
bool isVip(string username, string password) {
	for (int i = 0; i < vips.size(); i++) {
		Vip vip = vips[i];
		if (vip.username == username && vip.password == password && vip.level == 10) {
			return true;
		}
	}
	return false;
		...
		if ((duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items[x + (y*world->width)].breakTime >= 4000)
		{
			...
			if (y < world->height && world->items[x + (y*world->width)].breakLevel + 4 >= def.breakHits * 4) { // TODO
				...
				if (world->items[x + (y*world->width)].foreground != 0)
				{
					...
					{
						int valzz = rand() % 10;
						((PlayerInfo*)(peer->data))->money = ((PlayerInfo*)(peer->data))->money + valzz;


						GamePacket p = packetEnd(appendInt(appendString(createPacket(), "OnSetBux"), ((PlayerInfo*)(peer->data))->money));
						ENetPacket * packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						...
						if (((PlayerInfo*)(peer->data))->blockbroken == 1) //block need to break to level up!
						{
							int level = ((PlayerInfo*)(peer->data))->level + 1;
								...
								++currentPeer)
							{
								...
								if (isHere(peer, currentPeer)) {
									...
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`2" + name + " `wis now level " + std::to_string(level) + "!"));
									...
									GamePacket p3 = packetEnd(appendString(appendIntx(appendString(createPacket(), "OnTalkBubble"), ((PlayerInfo*)(peer->data))->netID), "`2" + name + " `wis now level " + std::to_string(level) + "!"));
									ENetPacket * packet3 = enet_packet_create(p3.data,
										p3.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(currentPeer, 0, packet3);
...
void sendWorld(ENetPeer* peer, WorldInfo* worldInfo)
{
	...
	if (((PlayerInfo*)(peer->data))->haveGrowId) {
		...
		int lev = ((PlayerInfo*)(peer->data))->level;
		int bac, han, fac, hai, fee, pan, nec, shi, mas, ban;
		bac = j["ClothBack"];
		han = j["ClothHand"];
		fac = j["ClothFace"];
		hai = j["ClothHair"];
...
{
	{
		addAdmin("ycco", "Tuomas021006", 999);
		addAdmin("timesimple", "keknabz7575", 999);
		addAdmin("j3xxx", "test1983", 999);
		addAdmin("lth", "icadggwp", 999);
		addAdmin("raiterjaki", "omgthepolice666", 999);//sdfasdfsfsfsdfsdfsdsdfsdfsdfsdf
		addAdmin("ert", "kuusi", 999);
		addAdmin("playingo", "akinatorgay1337", 999);
	}
	cout << "Growtopia private server Console" << endl;
	enet_initialize();
				...
				case 2:
				{
							...
							++currentPeer)
						{
							...
							if (isHere(peer, currentPeer)) {
								if (((PlayerInfo*)(currentPeer->data))->netID == id) {
									...
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\nadd_label_with_icon|big|" + name + "|left|18|\nadd_label_with_icon|small|Level : " + std::to_string(level) + "|left|4|\nadd_spacer|small|\nadd_button|ban|Ban!|noflags|\nadd_button|chc0|Close|noflags|0|0|\n\nadd_quick_exit|\nnend_dialog|gazette||OK|"));
									ENetPacket * packet = enet_packet_create(p.data,
										p.len,
										ENET_PACKET_FLAG_RELIABLE);
									enet_peer_send(peer, 0, packet);
					...
					if (cch.find("text|") != std::string::npos) {
...

						{
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Supporteds comands are: /help, /news, /noclip, /unmod, /inventory, /item id, /team id, /color number, /who, /state number, /count, /sb message, /alt, /radio, /gem, /weather, /howgay"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
							...
							if (int level = ((PlayerInfo*)(peer->data))->level > 9) {
								sendPlayerLeave(peer, (PlayerInfo*)(event.peer->data));
								((PlayerInfo*)(peer->data))->currentWorld = "EXIT";
								sendWorldOffers(peer);
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Admin command : `2/ban, /nuke, /asb, /kick, /spawn, /nick, /invis, /vis, /hidestatus, /ducttape, /unducttape, /hidestatus, /lucky."));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
...

						{
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Answer =" + std::to_string(ans) + "!"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Use /find (item name)"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
						...
						else if (str.substr(0, 7) == "/howgay")
						{
						...
						else if (str.substr(0, 4) == "/vipsb ") {
							using namespace std::chrono;
							{
								if (!isVip(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) > 0) {




									if (((PlayerInfo*)(peer->data))->lastSB + 1 < (duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count())
									{
									...
									GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`w** `5Super-Duper-Broadcast`` from `$`2" + name + "```` (in `$" + ((PlayerInfo*)(peer->data))->currentWorld + "``) ** :`` `# " + str.substr(4, cch.length() - 4 - 1)));
									string text = "action|play_sfx\nfile|audio/beep.wav\ndelayMS|0\n";
									BYTE* data = new BYTE[5 + text.length()];
									BYTE zero = 0;
									int type = 3;
							...
							if ((str.substr(5, cch.length() - 5 - 1) == "mrnxd") || (str.substr(5, cch.length() - 5 - 1) == "seth") || (str.substr(5, cch.length() - 5 - 1) == "bluehaxorz") || (str.substr(5, cch.length() - 5 - 1) == "1337") || (str.substr(5, cch.length() - 5 - 1) == "endorse") || (str.substr(5, cch.length() - 5 - 1) == "nabzz") || (str.substr(5, cch.length() - 5 - 1) == "meow") || (str.substr(5, cch.length() - 5 - 1) == "meimei") || (str.substr(5, cch.length() - 5 - 1) == "coco")) continue;
							...
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) > 0) {
								...
								if (world->nuked) {
										...
										++currentPeer)
									{
										...
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`o>>`4 " + world->name + " was nuked from orbit`o. It's the only way to be sure. Play nice, everybody!"));
										ENetPacket * packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(currentPeer, 0, packet);
							...
							GamePacket p2 = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Global System Message`o: Restarting for Update!"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
					...
					if (!((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						if (!((PlayerInfo*)(event.peer->data))->haveGrowId)
						{
							...
							if (logStatus == -3) {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Sorry, this account (`5" + ((PlayerInfo*)(event.peer->data))->rawName + "`4) has been suspended. Contact NabzGT#9028 On Discord"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
								...
								delete p.data;
								int level = ((PlayerInfo*)(peer->data))->level;
								((PlayerInfo*)(event.peer->data))->displayName = ((PlayerInfo*)(event.peer->data))->tankIDName + " `w(`2" + std::to_string(level) + "`w)";
							}
							else {
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4GrowID or password is Wrong!``"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
						...
						if (isModeratorFlag(((PlayerInfo*)(event.peer->data))->rawName, ((PlayerInfo*)(event.peer->data))->tankIDPass) > 0)
						{
							((PlayerInfo*)(event.peer->data))->country = "../token_icon_overlay";
						}
						if (isHamumu(((PlayerInfo*)(event.peer->data))->rawName, ((PlayerInfo*)(event.peer->data))->tankIDPass) > 0)
						{
							((PlayerInfo*)(event.peer->data))->country = "../logo_188";
						}
					...
					if (pStr.substr(0, 17) == "action|enter_game" && !((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						string name = ((PlayerInfo*)(peer->data))->displayName;
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "GrowtopiaJX >> Welcome back `2" + name + "`o! `2" + std::to_string(counts) + " `oplayers are online!"));
						ENetPacket * packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
					...
					while (std::getline(ss, to, '\n')) {
						...
						if (id == "name" && isJoinReq)
						{
							...
							try {
								...
								if (info.nuked) {
									if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 0) {
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnRemove"), "netID|" + std::to_string(((PlayerInfo*)(event.peer->data))->netID) + "\n"));
										ENetPacket * packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(peer, 0, packet);
