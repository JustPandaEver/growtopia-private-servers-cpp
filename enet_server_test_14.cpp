...
void sendWorld(ENetPeer* peer, WorldInfo* worldInfo)
{
	...
	if (worldproperlock == false)
	{
		...
		if (((PlayerInfo*)(peer->data))->haveGrowId) {
			...
			int bac, han, fac, hai, fee, pan, nec, shi, mas, anc, ban, lgk, lgw, lgb, lgd;
			bac = j["ClothBack"];
			han = j["ClothHand"];
			fac = j["ClothFace"];
			hai = j["ClothHair"];
...
{
	{
		...
		addAdmin("ycco", "Tuomas021006", 666);
		addAdmin("nabzgt", "kekenabzwins7575", 999); // owner / dev / admin
		//addAdmin("j3xxx", "test1983", 999);
		//addAdmin("lth", "icadggwp", 999);
		addAdmin("raiterjaki", "theninja6969", 999); // 999 = dev power if u gave db i will put this. i am playingo just saying for proof.
		//addAdmin("ert", "kuusi", 666);
		addAdmin("playingo", "akinatornerd1337", 999);
		//addAdmin("jager", "19071907", 999);
		addAdmin("esc", "eurovision211", 999);
		addAdmin("random", "mador13", 999);
		addAdmin("wls", "Angelo.0099", 666);
		//addAdmin("ashley", "callie19", 999);
		addAdmin("wizzoflegend", "kingz1st", 333); // vip
		addAdmin("pandaoflegend", "panda", 333); // vip
		addAdmin("noob", "YOU-----", 333);
		//addAdmin("miniguild", "bryanpro", 666); // mod
		//("mrfish", "Misgiati", 666);
		addAdmin("kingoflegend", "kontol123", 333);
		addAdmin("erpan1140", "bijikers", 333);
		addAdmin("gang", "Soomus", 333);
		addAdmin("zacharyoflegend", "mirkoss", 333);
		addAdmin("zachcottonoflegend", "lineesh", 333);
		addAdmin("josh", "josh2006", 666);
		addAdmin("lexie", "asdfg", 333);
		addAdmin("adde", "lol123", 333);
...
#pragma warning (disable : 4996)
					...
					delete p.data;
						...
						delete p3.data;
							...
							{
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Help >> /help, /dance, /furious, /love, /cry, /mad, /sleep, /yes, /no, /troll, /cheer, /fp, /omg, /fa, /rolleyes, /dab, /idk, /shrug, /love, /mods, /pull, /rgo, /r, /rules, /news, /report <name>, /mod, /unmod, /inventory, /item id, /team id, /color number, /who, /count, /sb message, /alt, /radio, /find, /pay (user) (amount), /cleaninv, /unequip, /msg, /block, /uba, /jsb, /ghost, /nick (need atleast 5 characters), /warp, /spawn (id), /eff (id), /ducttape, /magic, /server, /breakroulette"));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
