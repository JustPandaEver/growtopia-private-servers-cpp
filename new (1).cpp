		...
		if ((duration_cast<milliseconds>(system_clock::now().time_since_epoch())).count() - world->items[x + (y*world->width)].breakTime >= 4000)
		{
			...
			if (y < world->height && world->items[x + (y*world->width)].breakLevel + 4 >= def.breakHits * 4) { // TODO
				...
				if (world->items[x + (y*world->width)].foreground != 0)
				{
					...
					if (((PlayerInfo*)(peer->data))->xp >= 150) {
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	cout << "Growtopia private server (c) KEKE NABZ WINS" << endl;
	enet_initialize();
		...
		while (enet_host_service(server, &event, 1000) > 0)
		{
			...
			case ENET_EVENT_TYPE_CONNECT:
			{
					...
					++currentPeer)
				{
					...
					bool exist = std::experimental::filesystem::exists("devicebans/" + currentip + ".txt");



					if (exist == true)
					{
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Sorry, this device or location has been suspended. Contact NabzGT#9028 On Discord"));
						ENetPacket * packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
				...
				case 2:
				{
							...
							vector<string> infoDat = explode("|", to);
							{
								...
								if (dialog_name == "captcha") {
									if (btn == pData->buttonID) {
										...
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnDialogRequest"), "set_default_color|`o\n\nadd_label_with_icon|big|Growtopia Gazette``|left|5016|\n\nadd_spacer|small|\n\nadd_spacer|small|\nadd_label|small|`6Welcome To GrowtopiaJX!|left|4|\nadd_spacer|small||\n\nadd_textbox|`3Easter Update!|left|4|\nadd_label_with_icon|small|`3Fixed Server Crashes|left|610|\n\nadd_textbox|`32. Added ip ban for mods!|left|4|\n\nadd_textbox|`33. if u want find items do /find (item name)|left|4|\n\nadd_spacer|small|\n\nadd_textbox|`Join our Discord for more information!|left|4|\nadd_spacer|small|\nadd_label|small|`$- This Server is hosted by J3XXX.|left|4|\nadd_spacer|small|\nadd_label|small|`4Make sure to Join Our discord server!|left|4|\nadd_url_button||``New discord server!``|NOFLAGS|https://discord.gg/SRyfNR3| Join discord server?|0|0|\nadd_quick_exit|\nadd_button|chc0|Close|noflags|0|0|\nnend_dialog|gazette||OK|"));
										ENetPacket * packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(peer, 0, packet);
					...
					if (cch.find("text|") != std::string::npos) {
							...
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) > 0) {
								...
								if (world->nuked) {
										...
										++currentPeer)
									{
										...
										GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4" + world->name + " has been nuked!"));
										ENetPacket * packet = enet_packet_create(p.data,
											p.len,
											ENET_PACKET_FLAG_RELIABLE);
										enet_peer_send(currentPeer, 0, packet);
						...
						if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999)
						{
							...
							GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4Device with ID:\n " + x + " added to ban list. Abusing this command will lead into ban and demote!\n"));
							ENetPacket * packet = enet_packet_create(p.data,
								p.len,
								ENET_PACKET_FLAG_RELIABLE);
							enet_peer_send(peer, 0, packet);
							...
							if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 999)
							{
								string playerCalled = str.substr(6, cch.length() - 6 - 1);
								...
								GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "``Player infos: " + x));
								ENetPacket * packet = enet_packet_create(p.data,
									p.len,
									ENET_PACKET_FLAG_RELIABLE);
								enet_peer_send(peer, 0, packet);
					...
					if (!((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						if (!((PlayerInfo*)(event.peer->data))->haveGrowId)
						{
							...
							if (logStatus == -3) {
								...
								((PlayerInfo*)(event.peer->data))->displayName = ((PlayerInfo*)(event.peer->data))->tankIDName + " `3(Player)";
								if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) > 0) {
									((PlayerInfo*)(event.peer->data))->displayName = "`3[Server Admin] `w" + ((PlayerInfo*)(event.peer->data))->tankIDName;
								}
							}
							else {
					...
					if (pStr.substr(0, 17) == "action|enter_game" && !((PlayerInfo*)(event.peer->data))->isIn)
					{
						...
						string name = ((PlayerInfo*)(peer->data))->displayName;
						GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "Welcome back `w" + name + "`w! `w" + std::to_string(counts) + " `wplayers are online!"));
						ENetPacket * packet = enet_packet_create(p.data,
							p.len,
							ENET_PACKET_FLAG_RELIABLE);
						enet_peer_send(peer, 0, packet);
							...
							try {
								...
								if (info.nuked) {
									if (getAdminLevel(((PlayerInfo*)(peer->data))->rawName, ((PlayerInfo*)(peer->data))->tankIDPass) == 0) {
										...
										sendWorldOffers(peer);
										{
											GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`4This world is nuked!"));
											ENetPacket * packet = enet_packet_create(p.data,
												p.len,
												ENET_PACKET_FLAG_RELIABLE);
											enet_peer_send(peer, 0, packet);
